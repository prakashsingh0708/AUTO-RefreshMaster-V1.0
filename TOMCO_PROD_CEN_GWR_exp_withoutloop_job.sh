...
if (( $(echo "$size > $estimation" | bc -l) )); then
	echo "Size from df ($size G) is greater than Estimation from expdp ($estimation G). Initiating Export job..." >> /opt/oracle/refresh/logs/status.log
...
rsync -av --delete-before -c --checksum  -e "sshpass -p '$ORACLE_PASSWORD' ssh" /oradump/TOMCO_PROD_cen_gwr_exp*.dmp 10.245.93.100:/oradump/refresh/
    ...
    echo "ALERT!! Dear, There is Space Crunch in the export DIR, Please do the housekeeping! Size from df: $size G, Estimation from expdp: $estimation G" >> /opt/oracle/refresh/logs/status.log
    SUBJECT="[Gold][CRITICAL][NOT-STARTED][DB]Cloud_S_GOLD-oracle PROD CEN_GWR_exp JOB NOT-STARTED"	
fi
